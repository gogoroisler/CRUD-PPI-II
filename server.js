  ...
  fs.createReadStream(CSV_FILE) // Crea un flujo de lectura para el archivo CSV
    ...
    .on("end", () => {
      ...
      res.status(201).json({
        ...
        record: newRecord,
      }); // Responde con el nuevo registro
    })
    .on("error", (err) => {
      console.error(err); // Imprime el error en la consola
...
app.delete("/api/data/:id", (req, res) => {
  ...
  fs.createReadStream(CSV_FILE)
    ...
    .on("data", (data) => {
      if (data.id !== id) {
        // Solo almacena registros que no coincidan con el ID
        records.push(data);
      }
    })
    .on("end", () => {
...
app.put("/api/data/:id", (req, res) => {
  ...
  fs.createReadStream(CSV_FILE) // Crea un flujo de lectura para el archivo CSV
    ...
    .on("data", (data) => {
      if (data.id === id) {
        // Si el ID coincide, almacena el registro actualizado
